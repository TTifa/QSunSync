    ...
    public class SyncRecord
    {
        ...
        public static void CreateSyncRecordDB(string jobsDbPath)
        {
            ...
            string sqlCreate = new StringBuilder()
                ...
                .Append("[skip_dir] VARCHAR(500) NOT NULL,")
        ...
        public static void RecordSyncJob(string syncId, DateTime syncDateTime, SyncSetting syncSetting, string jobsDbPath)
        {
            ...
            string queryInsert = new StringBuilder().Append("INSERT INTO [sync_jobs] ([sync_id], [sync_local_dir], [sync_target_bucket], [file_type], [check_remote_duplicate], ")
                ...
                .Append("[upload_entry_domain], [sync_date_time], [skip_dir]) VALUES ( @sync_id, @sync_local_dir, @sync_target_bucket, @file_type, @check_remote_duplicate, @sync_prefix, @check_new_files, @ignore_dir, ")
                .Append("@skip_prefixes, @skip_suffixes, @overwrite_file, @default_chunk_size, @chunk_upload_threshold, @sync_thread_count, @upload_entry_domain, @sync_date_time, @skip_dir)").ToString();
            using (SQLiteConnection sqlCon = new SQLiteConnection(conStr))
            {
                sqlCon.Open();
                using (SQLiteTransaction sqlTrans = sqlCon.BeginTransaction())
                {
                    using (SQLiteCommand sqlCmd = new SQLiteCommand(sqlCon))
                    {
                        ...
                        sqlCmd.Parameters.Add("@skip_dir", System.Data.DbType.String);
                        ...
                        sqlCmd.Parameters["@skip_dir"].Value = syncSetting.SkipDir;
